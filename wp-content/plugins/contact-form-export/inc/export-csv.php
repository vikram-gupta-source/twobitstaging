...
class Expoert_CSV{
    ...
    public function get_email_listing() {
      $data = [];
      $rows = $this->cfdb->get_results("SELECT form_value FROM $this->table_name WHERE form_post_id = '$this->fid' AND DATE_FORMAT(form_date, '%Y-%m-%d') = (CURDATE() - INTERVAL 7 DAY) ORDER BY form_id DESC", OBJECT);
      foreach($rows as $row) {
    ...
    public function send_csv_email() {
      $sendto = get_field('daily_email_address', 'options');
      if(!empty($sendto)) {
