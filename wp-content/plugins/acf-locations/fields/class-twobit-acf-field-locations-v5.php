...
if( ! class_exists('twbit_field_select') ) :

class twbit_field_select extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		// vars
		$this->name = 'location';
		$this->label = _x('Locations', 'noun', 'acf');
	...
	*/
    ...
    $_choices = [ 'all|all' => '--- All Locations ---' ];
    $locations = get_field('location_selection', 'option');
    foreach($locations as $location) {
      $_choices[$location['city'].'|'.$location['region']] = $location['city'];
    }
    $choices = $_choices;
	...
	*/
    ...
    $field['choices'] = acf_encode_choices($_choices);
...
// initialize
new twbit_field_select( $this->settings );
